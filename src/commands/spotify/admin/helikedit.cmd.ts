...
export default class SpotifyLikeCMD extends IBEEPCommand {
    public messageTrigger: RegExp = /^!helikedit$/;
    ...
    public async exec(message: Message): Promise<any> {
        if (conditionUtils.meetsPermission(message, orHigher(TwitchPermissions.Moderator))) {
            ...
            if (isSaved) {
                return this.sender.sendMessage(`Song is already in liked songs.`, message.message_id);
            }

            await global.spotify.playback.save();
            return this.sender.sendMessage(`Song added to liked songs.`, message.message_id);
        }
    }
