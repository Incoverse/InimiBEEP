...
let pushupAdd = global.config.pushupIncrements.stoneface


export default class StoneFailCMD extends IBEEPCommand {
    public messageTrigger: RegExp = /^!stonefail$/;
    ...
    public setup(): Promise<boolean | null> {
        ...
        global.commChannel.on(["stoneface:start", "stoneface:finish"], this.resetTracker);
        global.commChannel.on("stoneface:abort", this.removeNotedPushups);
    ...
    public unload(): Promise<boolean | null> {
        global.commChannel.off(["stoneface:start", "stoneface:finish"], this.resetTracker);
        global.commChannel.off("stoneface:abort", this.removeNotedPushups);
    ...
    public async exec(message: Message): Promise<any> {
        if (conditionUtils.meetsPermission(message, orHigher(TwitchPermissions.Helper))) {
            ...
            if (global.timers.stoneface && global.timers.stoneface.running) {
                ...
                this.sender.sendMessage(`@${this.broadcaster.SELF.display_name} failed to maintain the stoneface! (+${pushupAdd} pushup${pushupAdd==1?"":"s"})`, message.message_id);
            }
        }
    }
