...
let pushupAdd = global.config.pushupIncrements.wordBan


export default class SaidWordCMD extends IBEEPCommand {
    public messageTrigger: RegExp = /^!saidword$/;
    ...
    public setup(): Promise<boolean | null> {
        ...
        global.commChannel.on(["word-ban:start", "word-ban:finish"], this.resetTracker);
        global.commChannel.on("word-ban:abort", this.removeNotedPushups);
    ...
    public unload(): Promise<boolean | null> {
        global.commChannel.off(["word-ban:start", "word-ban:finish"], this.resetTracker);
        global.commChannel.off("word-ban:abort", this.removeNotedPushups);
