                ...
                switch (secondParameter.toLowerCase()) {
                    ...
                    case "extend":
                            ...
                            global.timers.accent.setTime(
                                new CronTime(new Date(global.timers.accent.nextDate().toJSDate().getTime() + (millisecondsLength) - Date.now()))
                            )
                        const prettyLength = formatDuration(millisecondsLength, true);
        
                        global.commChannel.emit("accent:extend", millisecondsLength);
                        await this.sender.sendMessage(`The ACCENT timer has been extended by ${prettyLength}!`, message.message_id);
                        break;
                    case "set":
                        length = message.message.text.match(this.messageTrigger)[2];
                        millisecondsLength = /^[0-9]*$/.test(length) ? parseInt(length)*1000 : Math.round(parseDuration(length));
                        ...
                        if (global.timers.accent && global.timers.accent.running) {
                            ...
                            global.commChannel.emit("accent:time-set", millisecondsLength);
                            return await this.sender.sendMessage(`The ACCENT timer has been set to ${prettyLength} minute${parseInt(length) == 1 ? "" : "s"}!`, message.message_id);
                        } else {
