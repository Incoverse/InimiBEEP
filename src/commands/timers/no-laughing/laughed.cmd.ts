...
let pushupAdd = global.config.pushupIncrements.noLaughing


export default class LaughedCMD extends IBEEPCommand {
    public messageTrigger: RegExp = /^!(laugh|laughed)$/;
    ...
    public setup(): Promise<boolean | null> {
        ...
        global.commChannel.on(["no-laughing:start", "no-laughing:finish"], this.resetTracker);
        global.commChannel.on("no-laughing:abort", this.removeNotedPushups);
    ...
    public unload(): Promise<boolean | null> {
        global.commChannel.off(["no-laughing:start", "no-laughing:finish"], this.resetTracker);
        global.commChannel.off("no-laughing:abort", this.removeNotedPushups);
