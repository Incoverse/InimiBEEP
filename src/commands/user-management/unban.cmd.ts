...
export default class UnbanCMD extends IBEEPCommand {
    public messageTrigger: RegExp = /^!(unban|pardon|you are forgiven, )\s+(\w+)$/i;
    ...
    public async exec(message: Message): Promise<any> {
        if (conditionUtils.meetsPermission(message, orHigher(TwitchPermissions.Moderator))) {   
            ...
            await this.broadcaster.unbanUser(user.id);
        }
    }
