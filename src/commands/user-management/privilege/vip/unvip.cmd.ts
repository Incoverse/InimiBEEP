...
export default class UnVIPCMD extends IBEEPCommand {
    public messageTrigger: RegExp = /^!unvip\s+(\w+)$/;
    ...
    public async exec(message: Message): Promise<any> {
        if (conditionUtils.meetsPermission(message, orHigher(TwitchPermissions.Moderator))) {   
            ...
            if (!isVIP) {
                await this.sender.sendMessage(`${user.display_name} is not a VIP`, message.message_id);
                return;
            }
